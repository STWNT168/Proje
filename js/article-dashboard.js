...
async function diagnostic(){
 try{const m=await PMVApi.diagnoseMaster(),s=await PMVApi.diagnoseStatus($('spm-date')?.value||new Date().toISOString().slice(0,10));console.log('ARTICLE_MASTER DIAGNOSTIC',m,'ARTICLE_STATUS DIAGNOSTIC',s);alert(`ARTICLE_MASTER rows: ${m.totalRows}\nMatching assigned PIN rows: ${m.matchingPinRows}\nMissing PIN rows: ${m.missingPinRows}\nMissing article key rows: ${m.missingKeyRows}\nAssigned PINs: ${(m.session.assignedPins||[]).join(', ')}\nARTICLE_STATUS rows: ${s.totalRows}\nStatus entries for date: ${s.statusEntriesForDate}`)}catch(e){alert(e.message)}
}
