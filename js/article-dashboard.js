...
function bindTable(id,countId){const t=$(id);if(!t)return;const all=t.querySelector('.article-select-all');all?.addEventListener('change',e=>{t.querySelectorAll('.article-check').forEach(x=>x.checked=e.target.checked);if($(countId))$(countId).textContent=`${selected(id).length} selected`});t.querySelectorAll('.article-check').forEach(x=>x.addEventListener('change',()=>{if($(countId))$(countId).textContent=`${selected(id).length} selected`}));t.querySelectorAll('.article-save').forEach(b=>b.addEventListener('click',()=>saveOne(b.dataset.key)))}
async function loadSpm(){
 try{
  const session=PMVApi.getSession();
  if(!session) throw new Error('Not authenticated. Please sign in again.');
  const d=await PMVApi.articles($('spm-date')?.value||new Date().toISOString().slice(0,10),$('article-search')?.value||'');
  rows=d.articles||[];window.__spmArticleRows=rows;
  if($('article-scope'))$('article-scope').textContent=`Office: ${d.officeName||''} · Assigned PIN codes: ${(d.assignedPins||[]).join(', ')||'Not configured'} · ${d.total??rows.length} articles visible`;
  renderSpm();
...
async function bulkSpm(){
 ...
 const status=$('spm-bulk-status')?.value||'Pending',remarksByKey={};keys.forEach(k=>remarksByKey[k]=document.querySelector(`.article-remarks[data-key="${CSS.escape(k)}"]`)?.value||'');
...
async function loadAdmin(){try{
 const session=PMVApi.getSession();if(!session)throw new Error('Not authenticated. Please sign in again.');
 const d=await PMVApi.adminArticles($('admin-date')?.value||new Date().toISOString().slice(0,10),$('admin-article-search')?.value||'');
 adminRows=d.articles||[];renderAdmin();if($('admin-article-status'))$('admin-article-status').textContent=`${d.total??adminRows.length} records · Master rows: ${d.diagnostics?.masterRows??'?'} · Status rows: ${d.diagnostics?.statusRows??'?'}`;
...
async function diagnostic(){try{const p=await PMVApi.diagnosePinAccess(),m=await PMVApi.diagnoseMaster(),s=await PMVApi.diagnoseStatus($('spm-date')?.value||new Date().toISOString().slice(0,10));console.log('PIN',p,'MASTER',m,'STATUS',s);alert(`Office: ${p.officeName||''}\nSession PINs: ${(p.sessionPins||[]).join(', ')||'none'}\nUSER_MASTER PINs: ${(p.userPins||[]).join(', ')||'none'}\nOFFICE_MASTER PINs: ${(p.officePins||[]).join(', ')||'none'}\nEffective PINs: ${(p.effectivePins||[]).join(', ')||'none'}\nMatching articles: ${p.matchingArticles}\nARTICLE_MASTER rows: ${m.totalRows}\nARTICLE_STATUS rows: ${s.totalRows}`)}catch(e){alert(e.message)}}

const api={
 loadSpm,loadAdmin,bulkSpm,pushSelected,renderSpm,renderAdmin,diagnostic,
 // Compatibility expected by js/app.js
 bind(){},
 loadAdminArticles:loadAdmin
};
window.ArticleDashboard=api;
window.PMVArticles=api;

window.addEventListener('pmv-session-ready',()=>{
 if(document.readyState!=='loading'){
   const role=PMVApi.getSession()?.role;
   if(role==='SPM')loadSpm().catch(()=>{});
   else if(role==='ADMIN'||role==='DPS')loadAdmin().catch(()=>{});
 }
});
window.addEventListener('pmv-session-expired',e=>{
 const msg=e.detail?.message||'Session expired. Please sign in again.';
 if(window.App?.showLogin) window.App.showLogin(msg);
});
...
document.addEventListener('DOMContentLoaded',()=>{
 ...
 $('admin-article-fetch')?.addEventListener('click',loadAdmin);
