...
function renderAdmin(){
 ...
 t.innerHTML=`<thead><tr><th><input class="article-select-all" type="checkbox"></th><th>Barcode</th><th>PMV Application</th><th>Artisan</th><th>PIN</th><th>Office</th><th>SPM</th><th>Present</th><th>Master</th><th>Review</th><th>Updated By</th><th>Updated At</th><th>Source</th></tr></thead><tbody>${data.map(r=>`<tr><td><input class="article-check" data-key="${esc(r.articleKey)}" type="checkbox"></td><td>${esc(r.barCodeId)}</td><td>${esc(r.pmvApplicationNumber)}</td><td>${esc(r.artisanName)}</td><td>${esc(r.pinCode)}</td><td>${esc(r.officeName)}</td><td>${esc(r.spmId)}</td><td>${esc(norm(r.presentStatus))}</td><td>${esc(norm(r.masterStatus))}</td><td>${esc(r.reviewStatus||'PENDING')}</td><td>${esc(r.spmId||'')}</td><td>${esc(r.updatedAt||'')}</td><td>${esc(r.statusSource||'ARTICLE_MASTER')}</td></tr>`).join('')}</tbody>`;
 bindTable('adminArticles','admin-selected-count');renderSummary(data,'admin-status-summary');
}
async function loadAdmin(){try{
  const session=PMVApi.getSession();
  if(!session) throw new Error('Not authenticated. Please sign in again.');const d=await PMVApi.adminArticles($('admin-date').value,$('admin-article-search').value.trim());adminRows=d.articles||[];renderAdmin();if($('admin-article-status'))$('admin-article-status').textContent=`${d.total??adminRows.length} records · Master rows: ${d.diagnostics?.masterRows??'?'} · Status rows: ${d.diagnostics?.statusRows??'?'} · Matched: ${d.diagnostics?.matchedStatusRows??0}`}catch(e){alert(e.message)}}
