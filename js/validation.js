const PMVValidation = (() => {
  const ids=["received-kits","received-articles","redirected-kits","redirected-articles","rts-kits","rts-articles","delivered-kits","delivered-articles","invalid-mobile-kits","invalid-mobile-articles","torn-kits","torn-articles","improper-details-kits","improper-details-articles","deliverable-kits","deliverable-articles","incomplete-kits","incomplete-articles"];
  function validate(){
    const c=PMVCalc.compute(),errors=[];
    ids.forEach(id=>{const n=Number(document.getElementById(id)?.value);if(!Number.isInteger(n)||n<0)errors.push(id+" must be a non-negative whole number.");});
    ...
    if(box){box.innerHTML=errors.length?errors.map(x=>`<div>⚠ ${esc(x)}</div>`).join(""):"✓ All kit/article balances are valid.";box.className="validation "+(errors.length?"bad":"good");}
    const submit=document.getElementById("submit-report");if(submit)submit.disabled=errors.length>0;
